---
title: "Depuración de una lista de especies de plantas"
format: 
  html:
    lang: es
    theme: cosmo
...
ARCHIVO_LISTA_ESPECIES_ORIGINAL <- 
  here("data", "raw", "Lista de FLORA sin exóticas.xlsx")
...
lista_especies_01_depurada_basica_02_columnas_reducidas <-
  ...
  dplyr::select(
    species, speciesKey,
    CITES, iucnRedListCategory, `SINAC R- SINAC-CONAC-092-2017`, `25700-MINAE`
  ) |>
  rename(
    category_cites = CITES,
    category_iucn = iucnRedListCategory,
    category_sinac = `SINAC R- SINAC-CONAC-092-2017`,
    category_minae = `25700-MINAE`
  )
```
...
lista_especies_01_depurada_basica_04_columnas_conservacion_corregidas <- 
  ...
  mutate(
    category_cites = case_when(
      ...
      category_cites == 'II/NC' ~ NA,
      category_cites == 'No hay información de CITES' ~ NA,
      TRUE ~ category_cites
    )
  )
```
...
**Valores en la columna de categorías de MINAE**

```{r}
#| label: visualizacion-valores-minae
#| warning: false
#| message: false
#| code-fold: true
#| code-summary: "Código para la visualización de valores de la columna de categorías de MINAE"

# Valores de la columna
unique(lista_especies_01_depurada_basica_04_columnas_conservacion_corregidas$category_minae)
```
...
lista_especies_01_depurada_basica_05_final_01_ortografia_corregida <- 
  ...
  mutate(
    name = case_when(
      name == 'Cycnoches tondozii' ~ "Cycnoches tonduzii",
      name == 'Cycnoches ventricossum' ~ "Cycnoches ventricosum",
      name == 'Macrademia brassavolae' ~ "Macradenia brassavolae",
      name == 'Oncidium schoederianum' ~ "Oncidium schroederianum",
      name == 'Rossioglossum shlieperianum' ~ "Rossioglossum schlieperianum",
      TRUE ~ name
    )
  )
```
...
```{r}
#| label: depuracion-taxonomica-borrado-duplicados
#| warning: false
#| message: false
#| code-fold: true
#| code-summary: "Código para el borrado de registros"
...
# Borrado de registros con nombres de especies duplicados
# Se borran los que tienen NA es speciesKey (los que se agregaron de últimos)
# lista_especies_01_depurada_basica_05_final_02_duplicados_borrados <-
#   lista_especies_01_depurada_basica_05_final_01_ortografia_corregida |>
#   filter(!(name %in% nombres_especies_duplicados_3$name & is.na(speciesKey)))
...
```{r}
#| label: visualizacion-lista-nombres-especies-duplicados-4
#| code-fold: true
#| code-summary: "Código para la visualización de la lista"

# Generación de la lista de nombres de especies duplicados
nombres_especies_duplicados_4 <-
  lista_especies_01_depurada_basica_05_final_02_duplicados_borrados |>
  group_by(name) |>
  summarize(n = n()) |>
  filter(n > 1)
...
::: {.callout-note title="Cantidad de nombres de especies duplicados (para verificar el borrado)"}
`r nrow(nombres_especies_duplicados_4)`
:::

::: {.callout-note title="Cantidad de registros en la lista depurada de especies (después de borrar los duplicados)"}
`r nrow(lista_especies_01_depurada_basica_05_final_02_duplicados_borrados)`
:::
...
lista_especies_02_depurada_taxonomica_01_ortografia_corregida <- 
  name_backbone_checklist(
    name_data = lista_especies_01_depurada_basica_05_final_02_duplicados_borrados, 
    kingdom = REINO,
    strict = TRUE
)
```
...
lista_especies_03_final <-
  ...
  left_join(
    dplyr::select(
      lista_especies_01_depurada_basica_05_final_02_duplicados_borrados,
      name, category_cites, category_iucn, category_sinac, category_minae
    ),
...
# Correcciones finales en category_cites
lista_especies_03_final <- 
  lista_especies_03_final |>
  mutate(
    category_cites = case_when(
      canonicalName == 'Maxillaria egertoniana' ~ "II",
      TRUE ~ category_cites
    )
  )

# Correcciones finales en category_minae
lista_especies_03_final <- 
  lista_especies_03_final |>
  mutate(
    category_minae = case_when(
      canonicalName == 'Platymiscium parviflorum' ~ "PELIGRO EXTINCIÓN",
      canonicalName == 'Tachigali costaricensis' ~ "PELIGRO EXTINCIÓN",
      TRUE ~ category_minae
    )
  )
```
